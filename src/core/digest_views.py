...
Every response carries the token-link hygiene headers (TM-5): never indexed,
never cached, never leaks the token in a referrer. Expired-but-genuine links
get a friendly capability-free page; unknown and revoked tokens are the guard's
byte-identical 404 (T-TOKEN-2).
"""
...
def _resolve_or_respond(request: HttpRequest, raw_token: str) -> DigestToken | HttpResponse:
    ...
    try:
        ...
        return _harden(render(request, "core/digest_link_expired.html", status=410))
