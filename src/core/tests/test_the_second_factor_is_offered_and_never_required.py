...
def test_a_side_admin_is_not_told_their_sign_in_opens_every_side(
    ...
    `permissions.can_manage_member` stops them at what they can see, and the roster itself
    tells them so, one line under every row it will not let them touch ("Outside what you
    manage, so only <name> can change this").
    ...
    your side" was wrong for an admin whose own household belongs to both sides — one of
    the two people this is written for — and it named a second side to the other. What is
    true at either reach is what they can see.
    """
    prompt = _prompt(side_admin_client)
    assert "manages everyone" not in prompt, prompt
    assert "adds and removes the members you can see" in prompt, prompt
