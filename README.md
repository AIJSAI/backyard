...
- An elder path that requires no account and no app store: tap a link and you're in, in large single-column type, and photos and video work there too. Each post in the Family email carries a link that opens the app at the thread. (Replying to the e-mail itself was retired: nothing hands a relative a reply address, so a plain reply cannot be routed.) (Yes, a link that just works is a link that can be forwarded — that trade-off is argued out in [ADR-003](docs/adr/ADR-003-token-links.md).)
