...
- **PWA (S-103) and push (S-305, S-107):** manifest plus a deliberately minimal service worker; no app-shell precache. Web push SHIPPED in S-107, after v1: pywebpush with self-generated VAPID keys and declarative payloads, exactly as planned here, off unless the operator sets a key pair. S-305's e-mail reply nudge is unchanged and still the only thing that reaches a member who has subscribed no device; the two push event types (new posts, replies in a thread you are in) apply only to a device the member deliberately subscribed, and there is still no all-activity firehose. The elder token surface stays server-rendered and never depends on a service worker: Safari evicts a site's service worker after seven days of Safari use without a visit, and though an installed home-screen PWA is exempt, elders on a bare token link are the definition of intermittent visitors.
...
- Yard isolation (S-202) is application-layer discipline: membership-scoped default managers plus `get_object_or_404`, enforced by the isolation test suite as a merge gate from the first model. One missed scoped queryset is a cross-yard leak. Postgres row-level security as belt and suspenders was the Phase 2 decision this line flagged; it is now decided in [ADR-004](ADR-004-rls.md): deferred with triggers, with the role split and same-yard schema integrity built in wave 1 so the option stays cheap.
