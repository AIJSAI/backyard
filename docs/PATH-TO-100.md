...
> on one, and each says which. Where a box has an issue, it names it — the ones open at the
> time of writing are 176, 180, 181, 182, 187, 188, 191, 192, 193, 194 and 195, and that
> list is a snapshot rather than a register: ask the tracker.
...
- [x] Secrets hygiene verified: zero secrets in history, gitleaks full-history scan + continuous CI job with non-vacuous selftest evidence: docs/receipts/2026-07-20-secrets-scan.md — **QUALIFIED 2026-07-29.** "Zero secrets in history" was too strong, and the gate was blind in a specific, knowable way. `scripts/demo_seed.py` carried `PW = "<a fixed password a person chose>"` from the beginning, and that password **worked on the live production instance** as a seeded member — so a working credential sat in the public repository while this box was checked. gitleaks reported "no leaks found" across 287 commits and was right by its own rules: a human-chosen password assigned to `PW` looks nothing like a provider API key, and the selftest that proves the scan non-vacuous plants an **AWS-shaped** key, so it proves the scanner catches the class this was not. The literal is gone (generated per run, printed once), the two other copies of the same shape hiding in `os.environ.get(KEY, "literal")` fallbacks are gone with it, and the enforcing mechanism is now `src/core/tests/test_no_hardcoded_demo_credentials.py` — an `ast` check, so a comment beside the line cannot defeat it, proven to fire on both shapes. Kept checked rather than reopened because the original claim (no provider keys, no `.env`, history clean) still holds; the qualification is that a scanner tuned for high-entropy keys never covered the password a person picks, which is the likeliest thing a self-hoster commits. evidence: docs/receipts/2026-07-29-public-repo-hardening.md
