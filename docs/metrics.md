...
|---|---|---|
| Households that posted | Distinct pods that posted this week. Stored as `YardWeekMetrics.posting_breadth`, which is what the model, the migration and `rollup_metrics` still call it. The screen says "Households that posted" because the number counts pods, not people — three people in one household posting counts one — and because "posting breadth" was jargon on a page a relative reads. An ad-hoc group counts as one too, so the label is slightly generous, never an overcount | A feed with one poster is a broadcast, not a backyard; breadth predicts return visits |
