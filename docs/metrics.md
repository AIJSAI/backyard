...
| Reciprocity rate | Share of posts receiving any response (reaction or comment) within 7 days. A reply that arrived by email counts as a comment; no update has carried a reply address since 0.1.2, so none arrive today | The field-research failure mode was one-way flow; responses close the loop that keeps posters posting (R2) |
| Elder touch rate | Share of elder members with any touch this week (no-login link, email-update open proxy). The rollup still counts replies that arrived by email (`via_email`), which is zero while no update carries a reply address | The hardest segment; if elders connect, the design is working (R3) |
