...
| You look after a side of the family. | You are an Admin. You can add and remove the members you can see. (The owner's own rewrite was "You are a Side Admin. You can add and remove members on your side."; the role was renamed and the reach restated on 2026-09-20, see the glossary.) |
...
| roles | Member, Admin, Family Admin | plainly: "Admin: adds and removes the members they can see." "Family Admin: manages everyone and runs this Backyard." Never "look after". **"Side Admin" was retired on 2026-09-20** and is banned in `copy_scan.py`, for two reasons: one of the two relatives being handed these controls lives in a household that belongs to both sides, so her reach already covers both and "Side Admin" was the wrong word for her, while the other manages one side; and the name told a reader on one side that a second side exists. An admin's reach is the ordinary members they can see, which is true at every reach and needs no side named |
