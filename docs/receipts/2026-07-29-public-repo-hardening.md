...
- `scripts/demo_seed.py` mints `secrets.token_urlsafe(12)` per run and prints
  `DEMO_PASSWORD=…` as its last line. Verified end to end — a fresh seed printed
  `DEMO_PASSWORD=AFf43qWT02b6KDIc` and the screenshots below were captured by signing in with it.
