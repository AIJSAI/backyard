...
|---|---|
| `v0.3.2` | ✅ current |
| `v0.3.1` | ⚠️ superseded — installable and safe; `v0.3.2` moves the home-screen offer to a once-only card on every signed-in phone page (no migration) |
...
| `v0.1.4` | ⚠️ superseded — upgrade to `v0.3.2`: `v0.1.5` stopped an address being confirmed by anybody but its own signed-in account, which also gates password reset and Email Updates (CHANGELOG, Security) |
| `v0.1.3` | ⚠️ superseded — installable; upgrade to `v0.3.2`; the Security fixes in `v0.1.5` apply to every earlier tag |
| `v0.1.2` | ⚠️ superseded — installable; upgrade to `v0.3.2`; the Security fixes in `v0.1.5` apply to every earlier tag |
| `main` | ⚠️ moving target — fixes land here first, but it may be mid-refactor when you arrive |
| `v0.1.1` | ⚠️ superseded — installable, but its README's own install command fails; use `v0.3.2` |
| `v0.1.0` | ❌ withdrawn — superseded by `v0.3.2`; do not install |
